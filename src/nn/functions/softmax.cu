...
static __global__ void BM_KERNEL(softmax)(
    ...
    T* output,       // (batch, n)
    float temperature) {
    size_t offset = size_t(blockIdx.x) * size_t(n);
    logits += offset;
    output += offset;
    ...
    for (int i = threadIdx.x; i < n; i += blockDim.x) {
        local_sum += expf((float) logits[i] / temperature - local_max);
    }
    local_sum = functions::blockReduceSum<float>(local_sum);
    for (int i = threadIdx.x; i < n; i += blockDim.x) {
        output[i] = expf((float) logits[i] / temperature - local_max) / local_sum;
    }
}
