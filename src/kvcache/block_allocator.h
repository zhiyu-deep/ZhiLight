...
struct PageConfig {
    ...
    PageConfig(size_t num_blocks) : PageConfig(num_blocks, DEFAULT_BLOCK_SIZE) { }
...
public:
    ...
    std::vector<int32_t> tokens() { return segment; }
    bool full();
    ...
    bool has_child(std::vector<int32_t> tokens) { return children.count(tokens) > 0; };
...
public:
    ...
    BlockTrieNode* trie_root() { return trie_root_.get(); }
