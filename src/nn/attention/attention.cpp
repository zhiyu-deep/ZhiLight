...
class Attention::impl::Int8Impl : public Attention::impl::NormalImpl {
public:
    ...
    Int8Impl(
        ...
        model::QuantConfig quant_config,
        bool parallel)
        : Attention::impl::NormalImpl(ctx, cfg, quant_config, parallel),
          quant_config(quant_config),
          parallel(parallel) { }
    virtual ~Int8Impl() = default;

    // clang-format off
    std::tuple<core::Tensor, core::Tensor> rotary_embedding_q_k(
        const core::Context& ctx,
        const core::Tensor& pos, // (batch?, seq_len)
        const core::Tensor& h_q, // (batch?, seq_len, num_heads * dim_head)
        const core::Tensor& h_k  // (batch?, seq_len, num_kv_heads * dim_head)
    ) {
        ctx.recordEvent("rotary_embedding");
        BM_ASSERT_EQ(h_q.dtype(), dtype, "Wrong h_q type; h_q needs be quant_back");
        BM_ASSERT_EQ(h_k.dtype(), dtype, "Wrong h_k type; h_k needs be quant_back");
        return rotary_embedding(ctx, pos, h_q, h_k);
    }
    void copy_kv_to_buffer(
        const core::Context& ctx,
        uint32_t len_q,
        uint32_t len_buf,
        const Tensor& h_k,
        const Tensor& h_v,
        const Tensor* scale_h,
        const Tensor* placement,
        const Tensor& key_buf,
        const Tensor& val_buf
        ) {
        core::EventScope event_scope(ctx, "copy_kv_to_buffer");
        cudaStream_t stream = ctx.current_stream()->ptr;
        const Tensor* scale_k = project_k.get_weight_scale();
        const Tensor* scale_v = project_v.get_weight_scale();
        if (h_k.dtype() == DataType::kInt32) {
            int8_op::quant_back_copy_to_buffer(
                ctx, num_kv_heads, len_q, len_buf, dim_head, placement, h_k, scale_h, scale_k, key_buf);
        } else {
            copy_to_buffer(num_kv_heads, len_q, len_buf, dim_head, placement, h_k, key_buf, stream);
        }
        if (h_v.dtype() == DataType::kInt32) {
            int8_op::quant_back_copy_to_buffer(
                ctx, num_kv_heads, len_q, len_buf, dim_head, placement, h_v, scale_h, scale_v, val_buf);
        } else {
            copy_to_buffer(num_kv_heads, len_q, len_buf, dim_head, placement, h_v, val_buf, stream);
        }
    }
    Tensor transpose_q_2_1(
        const core::Context& ctx,
        size_t batch,
        uint32_t len_q,
        const Tensor& h_q, // (batch?, len_q, num_heads * dim_head)
        const Tensor* scale_h,
        int event_level=2
    ) {
        if (h_q.dtype() == DataType::kInt32) {
            ctx.recordEvent("quant_back_transpose");
            return int8_op::quant_back_transpose(ctx, h_q, scale_h, project_q.get_weight_scale());
        } else {
            return transpose_q(ctx, h_q, len_q, event_level);
        }
    }

    bool qkv_forward(
        model::ModelContext& ctx,
        const Tensor& hidden_q,
        Tensor& h_q,
        Tensor& h_k,
        Tensor& h_v);

    Tensor dynamic_batch_forward(
        model::ModelContext& ctx,
        const Tensor& hidden_q, // (group_len_q, dim_model)
        const core::Tensor& position_or_bias);
    ...
    core::Tensor forward(
        ...
        if (m_ctx && m_ctx->dyn_batch()) {
            return dynamic_batch_forward(*m_ctx, hidden_q, position_bias);
        }
        ...
        check_past_kv_dev(ctx.active_device(), &key_buf, &val_buf, placement);

        bool quant_back = pos_bias_type == "rotary" || len_q > 8;
//        quant_back = true;
        Tensor h_q = project_q(ctx, hidden_q, quant_back); // (batch, len_q, num_heads * dim_head)
        Tensor h_k = project_k(ctx, hidden_q, quant_back); // (batch, len_q, num_kv_heads*dim_head)
        Tensor h_v = project_v(ctx, hidden_q, len_q > 8);  // (batch, len_q, num_kv_heads*dim_head)
        Tensor* scale_h = hidden_q.quant_scale.get();
        if (pos_bias_type == "rotary") {
            auto h_qk = rotary_embedding_q_k(ctx, position_bias, h_q, h_k);
            h_q = std::get<0>(h_qk);
            h_k = std::get<1>(h_qk);
        }

        h_q = h_q.view({ batch, len_q, num_heads, dim_head });
        h_k = h_k.view({ batch, len_q, num_kv_heads, dim_head });
        h_v = h_v.view({ batch, len_q, num_kv_heads, dim_head });

        copy_kv_to_buffer(ctx, len_q, len_buf, h_k, h_v, scale_h, placement, key_buf, val_buf);

        // (batch, len_q, num_heads * dim_head) => (batch, num_heads, len_q, dim_head)
        h_q = transpose_q_2_1(ctx, batch, len_q, h_q, scale_h);

        // Q * K
        ctx.recordEvent("Q*K");
        Tensor attn_score = gemm_transB.forward(
            ctx,
            h_q.view({ batch, num_kv_heads, num_head_groups * len_q, dim_head }),
            key_buf // ColMajor: (batch, num_kv_heads, len_buf, dim_head)T
        );          // (batch, num_kv_heads, num_head_groups * len_q, len_buf)

        // attn_softmax in-place update attn_score
        ctx.recordEvent("attn_softmax");
        const Tensor& pos_bias = pos_bias_type == "relative" ? position_bias : core::Tensor();
        Tensor attn_score_q = attn_score.view({ batch, num_heads, len_q, len_buf });
        attn_softmax(ctx, attn_scale, attn_score_q, mask, pos_bias);
        ...
        // transpose: (batch, num_heads, len_q, dim_head) => (batch, len_q, num_heads, dim_head)
        ctx.recordEvent("transposeAV");
        Tensor attn_value_t = transpose_2_1(ctx, attn_res.view({ batch, num_heads, len_q, dim_head }));
        ctx.recordEvent("End>transposeAV");

        ShapeT attn_value_shape = (mask.ndim() == 2)
                                  ? ShapeT({len_q, num_heads * dim_head})
                                  : ShapeT({batch, len_q, num_heads * dim_head});
        // return (batch?, len_q, dim_model)
        Tensor ret = attn_out.forward(ctx, attn_value_t.view(attn_value_shape), parallel, output);
        // functions::check_numeric(ctx, output);
        return ret;
    }
};
bool Attention::impl::Int8Impl::qkv_forward(
    model::ModelContext& ctx,
    const Tensor& hidden_q,
    Tensor& h_q,
    Tensor& h_k,
    Tensor& h_v) {
    model::DynBatchContext* dyn_batch = ctx.dyn_batch().get();
    if (pos_bias_type == "rotary" && ctx.rag_buffer() && linear_qkv) {
        Tensor int8_ret = linear_qkv->forward(ctx, hidden_q, false);
        int dim_q = num_heads * dim_head;
        int dim_kv = num_kv_heads * dim_head;
        int8_op::quant_scale_back3(
            ctx, int8_ret, int8_ret.quant_scale.get(), linear_qkv->get_weight_scale(),
            dim_q, dim_kv, &h_q, &h_k, &h_v);
        return true;
    }
    return false;
}

Tensor Attention::impl::Int8Impl::dynamic_batch_forward(
    model::ModelContext& ctx,
    const Tensor& hidden_q,  // (group_len_q, dim_model)
    const core::Tensor& position_bias
) {
    ...
    Tensor g_h_q, g_h_k, g_h_v;
    bool has_encode = !dyn_batch->ev_batch.empty();
    if (!qkv_forward(ctx, hidden_q, g_h_q, g_h_k, g_h_v)) {
        g_h_q = project_q(ctx, hidden_q, true);  // (group_len_q, num_heads * dim_head)
        g_h_k = project_k(ctx, hidden_q, true);  // (group_len_q, num_kv_heads * dim_head)
        g_h_v = project_v(ctx, hidden_q, true);  // (group_len_q, num_kv_heads * dim_head)
    }
    Tensor* g_scale_h = hidden_q.quant_scale.get();         // (group_len_q,)

    if (pos_bias_type == "rotary") {
        auto h_qk = rotary_embedding_q_k(ctx, position_bias, g_h_q, g_h_k);
        g_h_q = std::get<0>(h_qk);
        g_h_k = std::get<1>(h_qk);
    }
    ...
    // search part
    const Tensor* s_placement = ctx.identity(&dyn_batch->s_placement, "s_placement");
    BM_ASSERT_EQ(2, s_placement->ndim(), "placement is not 2d");
    size_t batch = s_placement->size(0);
    size_t len_q = s_placement->size(1);
    ...
    Tensor output = attn_out(ctx, attn_val_g, parallel || !quant_config.fuse_block()); // return (group_len_q, dim_model)
    return output;
}
...
Attention::Attention(
    ...
    } else if (quant_cfg.is_int8()) {
        normal_impl = new impl::Int8Impl(ctx, cfg, quant_cfg, parallel);
    } else {
