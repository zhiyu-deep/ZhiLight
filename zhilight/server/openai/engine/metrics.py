...
class Metrics:
    ...
    def __init__(self, labelnames: List[str]):
        # Unregister any existing CPM collectors
        for collector in list(REGISTRY._collector_to_names):
            if hasattr(collector, "_name") and "cpm" in collector._name:
                REGISTRY.unregister(collector)

        # Config Information
        self.info_cache_config = Info(
            name='cpm:cache_config',
            documentation='information of cache_config')

        # System stats
        self.gauge_scheduler_running = Gauge(
            name="cpm:num_requests_running",
            documentation="Number of requests currently running on GPU.",
            labelnames=labelnames)

        self.gauge_scheduler_waiting = Gauge(
            name="cpm:num_requests_waiting",
            documentation="Number of requests waiting to be processed.",
            labelnames=labelnames)

        # Raw stats from last model iteration
        self.counter_prompt_tokens = Counter(
            name="cpm:prompt_tokens_total",
            documentation="Number of prefill tokens processed.",
            labelnames=labelnames)
        self.counter_generation_tokens = Counter(
            name="cpm:generation_tokens_total",
            documentation="Number of generation tokens processed.",
            labelnames=labelnames)
        self.histogram_time_to_first_token = Histogram(
            name="cpm:time_to_first_token_seconds",
        ...
        self.histogram_time_per_output_token = Histogram(
            name="cpm:time_per_output_token_seconds",
        ...
        self.histogram_e2e_request_latency = Histogram(
            name="cpm:e2e_request_latency_seconds",
        ...
        self.gauge_avg_prompt_throughput = Gauge(
            name="cpm:avg_prompt_throughput_toks_per_s",
            documentation="Average prefill throughput in tokens/s.",
            labelnames=labelnames,
        )
        self.gauge_avg_generation_throughput = Gauge(
            name="cpm:avg_generation_throughput_toks_per_s",
...
class Stats:
    ...
    def clear(self):
        ...
        self.time_to_first_tokens.clear()
        self.time_per_output_tokens.clear()
        self.time_e2e_requests.clear()
...
class StatLogger:
    ...
    def _log_prometheus(self, stats: Stats) -> None:
        ...
        self.metrics.gauge_scheduler_waiting.labels(**self.labels).set(
            stats.num_total - stats.num_running)
    ...
    def _log_prometheus_interval(self, prompt_throughput: float,
        ...
        # the CPM side. Moving forward, we should use counters like
    ...
    def log(self, stats: Stats) -> None:
        ...
        if self._local_interval_elapsed(stats.now):
            ...
            logger.info(
                ...
                f"Pending: {stats.num_total - stats.num_running} reqs")
