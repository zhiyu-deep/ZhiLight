...
//  int split_k_iters = 1;
  ...
  int N = blockDim.x * gridDim.x;  // 2, dim_out / 8
...
core::Tensor awq_dequantize(
    ...
    int thy)
{
    size_t in_c = _kernel.size(0);
    size_t qout_c = _kernel.size(1);
    size_t out_c = qout_c * 8;
    size_t G = in_c / _scaling_factors.size(0);
    ...
    core::Tensor _de_kernel = ctx.tensor({in_c, out_c}, _scaling_factors.dtype());

    auto kernel = reinterpret_cast<int*>(_kernel.data<int>());
    auto de_kernel = reinterpret_cast<half*>(_de_kernel.data<half>());
    auto scaling_factors = reinterpret_cast<half*>(_scaling_factors.data<half>());
    auto zeros = reinterpret_cast<int*>(_zeros.data<int>());

    dim3 num_blocks(x_blocks, y_blocks);
    dim3 threads_per_block(x_thread, y_thread);

    auto stream = ctx.current_stream()->ptr;
    nn::awq::dequantize_weights<<<num_blocks, threads_per_block, 0, stream>>>(
...
template<class T>
__global__ void KERNEL_sum_dim0(
    const T* input, // (x, n)
    T* out, // (n)
    int X,
    uint32_t N
...
{
    size_t num_in_feats = _in_feats.size(0);
    size_t num_in_channels = _in_feats.size(1);

    core::Tensor _out_feats = ctx.tensor({split_k_iters, num_in_feats, _kernel.size(1) * 8}, _in_feats.dtype());
    int num_out_feats = _out_feats.size(-2);
    int num_out_channels = _out_feats.size(-1);

    auto in_feats = reinterpret_cast<half*>(_in_feats.data<half>());
    auto kernel = reinterpret_cast<int*>(_kernel.data<int>());
    auto out_feats = reinterpret_cast<half*>(_out_feats.data<half>());
    auto scaling_factors = reinterpret_cast<half*>(_scaling_factors.data<half>());
    auto zeros = reinterpret_cast<int*>(_zeros.data<int>());
    ...
    if (num_out_channels % 128 == 0)
    {
        ...
        nn::awq::gemm_forward_4bit_cuda_m16nXk32<128><<<num_blocks, threads_per_block, 0, stream>>>(
    ...
    else if (num_out_channels % 64 == 0)
    {
        ...
        // threadIdx.y: i_factors[2] * j_factors[2]
        dim3 threads_per_block(32, 2);
        nn::awq::gemm_forward_4bit_cuda_m16nXk32<64><<<num_blocks, threads_per_block, 0, stream>>>(
            group_size, split_k_iters, in_feats, kernel, scaling_factors, zeros, num_in_feats, num_in_channels,
            num_out_channels, out_feats);
    }

    core::Tensor out = ctx.tensor({num_in_feats, _kernel.size(1) * 8}, _in_feats.dtype());
//    functions::check_numeric(ctx, _out_feats);
    KERNEL_sum_dim0<half><<<out.numel() / 32, 32, 0, stream>>>(
        _out_feats.data<half>(),
        out.data<half>(),
        split_k_iters,
        out.numel());
    return out;
