...
//  static constexpr auto w_type = vllm::ScalarType::from_id(w_type_id);
  ...
  constexpr int pack_factor = 32 / size_bits;
  ...
  constexpr int b_thread_vecs = size_bits == 4 ? 1 : 2;
  ...
  auto matmul = [&](int k) {
    if constexpr (has_zp) {
      ...
      if constexpr (size_bits == 4) {
        zp_quant_0 = frag_qzp[k % 2][0];
        zp_quant_1 = zp_quant_0 >> 8;
      } else {
//        static_assert(size_bits == 8);
//        zp_quant_0 = frag_qzp[k % 2][0];
...
bool is_valid_cache_size(thread_config_t const& th_config, int max_m_blocks,
  ...
  while (true) {
    ...
    max_m_blocks--;
    if (max_m_blocks == 0) {
      BM_ASSERT(false, str_cat("Unexpected m_blocks = ", m_blocks));
    }
  }
  ...
  BM_ASSERT(max_shared_mem / 2 > scales_cache_size, "");  // Sanity
...
// W_TYPE, THREAD_M_BLOCKS, THREAD_N_BLOCKS, THREAD_K_BLOCKS, HAS_ACT_ORDER, HAS_ZP, GROUP_BLOCKS, NUM_THREADS
...
void marlin_mm(const void* A, const void* B, void* C, void* C_tmp, void* s,
               ...
               int sms, int max_par, bool use_fp32_reduce, int max_shared_mem) {
  if (has_zp) {
    BM_ASSERT(
        q_type == vllm::kU4 || q_type == vllm::kU8,
        str_cat("q_type must be u4 or u8 when has_zp = True. Got = ", q_type.str()));
  } else {
    BM_ASSERT(
        q_type == vllm::kU4B8 || q_type == vllm::kU8B128,
        str_cat("q_type must be uint4b8 or uint8b128 when has_zp = False. Got = ",
        q_type.str()));
  }

  BM_ASSERT(prob_m > 0 && prob_n > 0 && prob_k > 0, str_cat("Invalid MNK = [", prob_m,
              ", ", prob_n, ", ", prob_k, "]"));
  ...
  BM_ASSERT(exec_cfg.max_m_blocks > 0 &&
...
//              ", max_shared_mem = ", max_shared_mem);
  ...
  BM_ASSERT(prob_n % thread_n == 0, str_cat("prob_n = ", prob_n,
              " is not divisible by thread_n = ", thread_n));
  BM_ASSERT(prob_k % thread_k == 0, str_cat("prob_k = ", prob_k,
              " is not divisible by thread_k = ", thread_k));

  int group_blocks = 0;
  if (has_act_order) {
    if (is_k_full) {
      BM_ASSERT(group_size != -1, "");
      group_blocks = group_size / 16;
      BM_ASSERT(prob_k % group_blocks == 0, str_cat("prob_k = ", prob_k,
                  " is not divisible by group_blocks = ", group_blocks));
    } else {
      BM_ASSERT(group_size == 0, "");
      group_blocks = 0;
    }
...
//    AWQ_CALL_IF(vllm::kU8, 4, 8, 128)
      ...
      BM_ASSERT(false, str_cat("Unsupported shapes: MNK = [", prob_m, ", ", prob_n,
                  ", ", prob_k, "]"));
...
core::Tensor gptq_marlin_gemm(
  ...
  vllm::ScalarType q_type = vllm::kU4B8;
  vllm::ScalarType* b_q_type = &q_type;
  int pack_factor = 32 / b_q_type->size_bits();

  // Verify A
  BM_ASSERT(a.size(0) == size_m, str_cat("Shape mismatch: a.size(0) = ", a.size(0),
              ", size_m = ", size_m));
  BM_ASSERT(a.size(1) == size_k, str_cat("Shape mismatch: a.size(1) = ", a.size(1),
              ", size_k = ", size_k));

  // Verify B
  BM_ASSERT(size_k % marlin::tile_size == 0, str_cat("size_k = ", size_k,
              " is not divisible by tile_size = ", marlin::tile_size));
  BM_ASSERT((size_k / marlin::tile_size) == b_q_weight.size(0),
            str_cat("Shape mismatch: b_q_weight.size(0) = ", b_q_weight.size(0),
              ", size_k = ", size_k, ", tile_size = ", marlin::tile_size));
  BM_ASSERT(b_q_weight.size(1) % marlin::tile_size == 0,
            str_cat("b_q_weight.size(1) = ", b_q_weight.size(1),
              " is not divisible by tile_size = ", marlin::tile_size));
  int actual_size_n = (b_q_weight.size(1) / marlin::tile_size) * pack_factor;
  BM_ASSERT(size_n == actual_size_n, str_cat("size_n = ", size_n,
              ", actual_size_n = ", actual_size_n));

  // Alloc buffers
  core::Tensor c = ctx.tensor({size_m, size_n}, a.dtype());
  core::Tensor a_tmp = ctx.tensor({size_m, size_k}, a.dtype());

  // Alloc C tmp buffer that is going to be used for the global reduce
  size_t reduce_max_m = marlin::determine_reduce_max_m(size_m, marlin::max_par);
    size_t reduce_n = size_n;
//  if (!use_fp32_reduce) {
//    reduce_max_m = 0;
//    reduce_n = 0;
//  }
  core::Tensor c_tmp = ctx.tensor({reduce_max_m, reduce_n}, core::DataType::kFloat);
  ...
  BM_ASSERT((g_idx.ndim() == 0 && perm.ndim() == 0) ||
                  (g_idx.size(0) == size_k && perm.size(0) == size_k),
            str_cat("Unexpected g_idx.size(0) = ", g_idx.size(0),
  ...
  int num_groups = -1;
  int group_size = -1;
  bool has_act_order = g_idx.ndim() != 0;

  int rank = b_scales.ndim();
  BM_ASSERT(rank == 2, str_cat("b_scales rank = ", rank, " is not 2"));
  BM_ASSERT(b_scales.size(1) == size_n, str_cat("b_scales dim 1 = ", b_scales.size(1),
              " is not size_n = ", size_n));
  num_groups = b_scales.size(0);

  if (has_act_order) {
    if (is_k_full) {
      BM_ASSERT(num_groups > 1, "For act_order, num_groups must be > 1");
      BM_ASSERT(size_k % num_groups == 0, str_cat("size_k = ", size_k,
    ...
    if (num_groups > 1) {
      BM_ASSERT(
          size_k % num_groups == 0, str_cat("size_k = ", size_k,
  ...
  if (has_zp) {
    ...
    BM_ASSERT(rank == 2, str_cat("b_zeros rank = ", rank, " is not 2"));
    BM_ASSERT(b_zeros.size(0) == num_groups,
              str_cat("b_zeros dim 0 = ", b_zeros.size(0),
                " is not num_groups = ", num_groups));
    BM_ASSERT(b_zeros.size(1) == size_n / pack_factor,
              str_cat("b_zeros dim 1 = ", b_scales.size(1),
                " is not size_n / pack_factor = ", size_n / pack_factor));
  }

  // Verify workspace size
  BM_ASSERT(size_n % marlin::min_thread_n == 0, str_cat("size_n = ", size_n,
              ", is not divisible by min_thread_n = ", marlin::min_thread_n));
  int min_workspace_size = (size_n / marlin::min_thread_n) * marlin::max_par;
  BM_ASSERT(workspace.numel() >= min_workspace_size,
            str_cat("workspace.numel = ", workspace.numel(),
              " is below min_workspace_size = ", min_workspace_size));

  if (sms == -1) {
      sms = ctx.get_mp_count();
  }
  int max_shared_mem = ctx.get_max_shared_memory();
  auto stream = ctx.current_stream()->ptr;

  int dev = 0;
  if (a.dtype() == core::DataType::kHalf) {
      if (ctx.debug() > 2) {
          auto exec_cfg =
              marlin::determine_thread_config(size_m, size_n, size_k, 4, group_size,
                                      has_act_order, is_k_full, max_shared_mem);
          std::cout << "M=" << size_m << ", N=" << size_n << ", K=" << size_k
            << ", TM=" << exec_cfg.max_m_blocks
            << ", TN=" << exec_cfg.tb_cfg.thread_n
            << ", TK=" << exec_cfg.tb_cfg.thread_k
            << ", Tnum=" << exec_cfg.tb_cfg.num_threads
            << std::endl;
      }

      marlin::marlin_mm<half>(
        a.data(), b_q_weight.data(), c.data(),
        c_tmp.data<float>(), b_scales.data(),
        b_zeros.data(), g_idx.data(), perm.data(),
        a_tmp.data(), size_m, size_n, size_k,
        workspace.data(), *b_q_type, has_act_order, is_k_full, has_zp,
        num_groups, group_size, dev, stream,
        thread_k, thread_n, sms, marlin::max_par, use_fp32_reduce, max_shared_mem);
...
//        thread_k, thread_n, sms, marlin::max_par, use_fp32_reduce, max_shared_mem);
    ...
    BM_ASSERT(false, "gpt_marlin_gemm only supports bfloat16 and float16");
  }
